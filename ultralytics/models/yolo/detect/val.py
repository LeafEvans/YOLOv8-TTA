...
class DetectionValidator(BaseValidator):
    ...
    def preprocess(self, batch):
        """Preprocesses batch of images for YOLO validation."""
        batch["img"] = batch["img"].to(self.device, non_blocking=True)
        batch["img"] = (batch["img"].half() if self.args.half else batch["img"].float()) / 255
        for k in ["batch_idx", "cls", "bboxes"]:
            batch[k] = batch[k].to(self.device)

        if self.tta_strategy is not None and self.args.tta:
    ...
    def _apply_tta(self, batch):
        """Apply test-time adaptation strategy during validation"""
        if not hasattr(self, "model") or self.tta_strategy is None:
            return
        try:
            with torch.no_grad():
                # Extract features using predefined layers
                features = self.tta_strategy.extract_features(batch["img"])
                if features is None:
                    LOGGER.warning("No features extracted for TTA")
                    return

                # Calculate test-time distribution statistics
                ...
                if self.tta_strategy.should_update(loss):
                    # Only update adaptor parameters
            ...
            LOGGER.error(f"Error in TTA during validation: {str(e)}")
    ...
    def init_metrics(self, model):
        ...
        if hasattr(model, "trainer") and hasattr(model.trainer, "tta_strategy"):
            if isinstance(model.trainer.tta_strategy, TTAStrategy):
                self.tta_strategy = model.trainer.tta_strategy
